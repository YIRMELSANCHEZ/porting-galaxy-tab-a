...
1. **`results/phase-5/HANDOFF-CURRENT-STATE.md`** -- live state, current candidate,
   next step, uncommitted source changes to preserve. It is the source of truth
   for "right now".
2. **`results/phase-5/LOG-CAPTURE-METHOD.md`** -- HOW to debug with ADB without
   running out of data (the previous agent could not read logs). Essential.
3. **`results/phase-5/ARTIFACTS.sha256`** -- verifiable hashes of the current
