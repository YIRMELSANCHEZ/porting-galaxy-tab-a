...
For the technical detail of each point, follow the links to `phase-6/`.
...
|---|---|---|
| Android 10 recovery on hardware | validated (V21) | phase-4 |
| Android 10 boot (system-as-root, fstab, binder, HIDL graphics) | stable | phase-5 |
| Power button / suspend / power off | sprdfb panel bug (no resume on A10) fixed | [gtexa-power-button-real-cause] - phase-6/POWER-BUTTON-FINDINGS.md |
| Wi-Fi + Bluetooth coexisting (Marlin/wcnd chip) | works (V70-V72, V89-V90) | phase-6/BLUETOOTH-FINDINGS.md, V89-WIFI-START-RECOVERY.md |
| Display performance (OSD/DISPC cap) | removed (V76) | phase-6/G1-FINDINGS.md |
| GLES 3.0 in software (SwiftAngle = SwiftShader as ANGLE driver) | works (V77-V88) | phase-6/V87-CLEAN-WIFI-ANGLE.md, V88-SWIFTANGLE-SCALED-EGL.md |
| Hardware H.264 video (OMX.sprd.h264.decoder) | works (V94b); NV12 in SwiftShader (V94) | phase-6/BUILD-V96-FINAL.md |
| Camera: HAL working, 2 cameras | works (V92 + V98) | phase-6/CAMERA-V98.md |
| Google Play Services via microG | integrated (V99), native LineageOS signature spoofing | phase-6/BUILD-V96-FINAL.md |
| Final general-purpose package | `...PHASE6-v96` | phase-6/BUILD-V96-FINAL.md |
...
- **CAM2 -- camera preview**: HAL OK, but the Spreadtrum gralloc rejects the preview buffers (API1:
  ...
  [phase-6/CAMERA-V98.md](phase-6/CAMERA-V98.md) (preview section). High difficulty, requires rebuilding and
...
- **microG**: open "microG Settings", check the self-check (signature spoofing green), enable device
  registration and Cloud Messaging, grant permissions. Detail in phase-6/BUILD-V96-FINAL.md section 3.
...
### Minor (low priority, see phase-6/TODO-ANALYSIS.md)
...
- Each `Vnn` has its `scripts/apply-vNN-*.py` and, in most cases, a note in `results/phase-6/`.
