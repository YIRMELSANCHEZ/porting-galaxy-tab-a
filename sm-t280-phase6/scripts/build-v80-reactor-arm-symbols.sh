#!/bin/bash
# V80: on top of V79. Patches Reactor (LLVMReactor.cpp), recompiles SwiftShader, rebuilds SwiftAngle +
# system.img, generates the test APK (adb install -r, without reflashing) and, unless QUICK=1, packages
# boot(V75) + system(V80). Run in WSL (lineage).
set -e
...
TEST=$WIN/sm-t280-phase6/apps/SwiftAngle-test-v80.apk
...
echo "=== 1) SwiftShader ==="
mka libEGL_swiftshader libGLESv2_swiftshader libGLESv1_CM_swiftshader 2>&1 | grep -E "error:|FAILED|build completed" | tail -8 | tee /tmp/v80-mka.log
grep -q "build completed" /tmp/v80-mka.log || { echo "V80_FAIL: SwiftShader does not compile"; exit 1; }
for m in EGL GLESv2 GLESv1_CM; do cp -f "$OUT/system/vendor/lib/egl/lib${m}_swiftshader.so" "$PKG/lib${m}_angle.so"; done
strings "$PKG/libGLESv2_angle.so" | grep -c "unresolved external symbol" | sed 's/^/libGLESv2_angle with log V80: /'

echo "=== 2) SwiftAngle + system.img ==="
mka SwiftAngle systemimage 2>&1 | grep -vE '^\[ *[0-9]+% ' | grep -E "error:|FAILED|build completed" | tail -5
test -f "$APK" || { echo "V80_FAIL: SwiftAngle.apk not built"; exit 1; }
unzip -v "$APK" | grep a4a_rules.json | grep -q "Stored" || { echo "V80_FAIL: compressed asset"; exit 1; }
