#!/bin/bash
# V81: on top of V80 (Reactor: host CPU/features in the JIT), recompiles SwiftShader, rebuilds SwiftAngle +
# system.img, generates the test APK (adb install -r, without reflashing) and, unless QUICK=1, packages
# boot(V75) + system(V81). Run in WSL (lineage).
set -e
...
TEST=$WIN/sm-t280-phase6/apps/SwiftAngle-test-v81.apk
...
echo "=== 1) SwiftShader ==="
mka libEGL_swiftshader libGLESv2_swiftshader libGLESv1_CM_swiftshader 2>&1 | grep -E "error:|FAILED|build completed" | tail -8 | tee /tmp/v81-mka.log
grep -q "build completed" /tmp/v81-mka.log || { echo "V81_FAIL: SwiftShader does not compile"; exit 1; }
for m in EGL GLESv2 GLESv1_CM; do cp -f "$OUT/system/vendor/lib/egl/lib${m}_swiftshader.so" "$PKG/lib${m}_angle.so"; done
strings "$PKG/libGLESv2_angle.so" | grep -c "unresolved external symbol" | sed 's/^/libGLESv2_angle with log V81: /'

echo "=== 2) SwiftAngle + system.img ==="
mka SwiftAngle systemimage 2>&1 | grep -vE '^\[ *[0-9]+% ' | grep -E "error:|FAILED|build completed" | tail -5
test -f "$APK" || { echo "V81_FAIL: SwiftAngle.apk not built"; exit 1; }
unzip -v "$APK" | grep a4a_rules.json | grep -q "Stored" || { echo "V81_FAIL: compressed asset"; exit 1; }
...
echo "=== 4) package (boot V75 + system V81) ==="
sha256sum "$OUT/boot.img" | cut -c1-16 | sed 's/^/boot.img (V75 fe89ef3181bfecea): /'
python3 "$S/legacy_sparse.py" "$OUT/system.img" "$WIN/results/phase-5/system-legacy-sparse-odin-v81.img"
bash "$S/package-system-for-odin.sh" "$OUT/boot.img" \
  "$WIN/results/phase-5/system-legacy-sparse-odin-v81.img" \
  "$WIN/sm-t280-phase6/packages/SM-T280-android10-swiftangle-PHASE6-v81-DO-NOT-FLASH.tar.md5"
