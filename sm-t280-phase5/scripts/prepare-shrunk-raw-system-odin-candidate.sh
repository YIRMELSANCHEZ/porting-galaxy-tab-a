...
raw="$workspace/results/phase-5/system-raw-shrunk-odin-v3.img"
...
bash "$workspace/sm-t280-phase5/scripts/package-system-for-odin.sh" \
  "$boot" "$raw" "$package"
tar --list --file "$package" | sed '/^$/d' > "$workspace/results/phase-5/SHRUNK-SYSTEM-V3-TAR-CONTENTS.txt"
test "$(cat "$workspace/results/phase-5/SHRUNK-SYSTEM-V3-TAR-CONTENTS.txt")" = $'boot.img\nsystem.img'
