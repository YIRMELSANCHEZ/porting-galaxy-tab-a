#!/usr/bin/env bash
# Offline-only rescue package: exact stock AQJ1 boot.img + system.img only.
set -euo pipefail

workspace=${1:?Usage: prepare-stock-boot-system-restore.sh WORKSPACE}
stock_ap="$workspace/sm-t280-phase4/stock/extracted/AP_T280XXU0AQJ1_CL10429622_QB15394287_REV00_user_low_ship.tar.md5"
output="$workspace/sm-t280-phase5/packages/SM-T280-AQJ1-STOCK-BOOT-SYSTEM-ONLY-RESTORE.tar.md5"
work=$(mktemp -d)
trap 'rm -rf -- "$work"' EXIT

test -f "$stock_ap"
tar --extract --file "$stock_ap" --directory "$work" boot.img system.img
test -s "$work/boot.img"
test -s "$work/system.img"

archive=$(basename "${output%.md5}")
(
  cd "$work"
  tar --format=ustar -cf "$archive" boot.img system.img
...
printf 'stock_system_sha256=%s\n' "$(sha256sum "$work/system.img" | awk '{print $1}')"
printf 'package_sha256=%s\n' "$(sha256sum "$output" | awk '{print $1}')"
printf 'STOCK_BOOT_SYSTEM_RESCUE_PACKAGE_PASS\n'
